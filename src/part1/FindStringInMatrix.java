...
public class FindStringInMatrix {
    ...
    private static boolean hasPathElement(char[] matrix, int rows, int cols, char[] str,
        ...
        if (i < 0 || i > rows || j < 0 || j > cols || matrix[index] != str[strIndex] || isVisited[index]==true)
            return false;
        if(strIndex==str.length-1)
            return true;

        isVisited[index]=true;//先将访问的矩阵元素状态置为“true”
        if(hasPathElement(matrix, rows, cols, str, i-1, j, ++strIndex, isVisited)
                ||hasPathElement(matrix, rows, cols, str, i+1, j, ++strIndex, isVisited)
                ||hasPathElement(matrix, rows, cols, str, i, j-1, ++strIndex, isVisited)
                ||hasPathElement(matrix, rows, cols, str, i, j+1, ++strIndex, isVisited))
            return true;
